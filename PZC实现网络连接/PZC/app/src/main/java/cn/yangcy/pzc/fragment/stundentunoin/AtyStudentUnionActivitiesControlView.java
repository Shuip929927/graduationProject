...
//        setContentView(R.layout.student_union_activities_control_view);
        ...
        if(NetWorkUtil.isConnected(getBaseContext())){
            activitiesLiveData = mViewModel.getActivitiesLiveByIdNet(activitiesId);
        } else {
        ...
        activitiesLiveData.observe(this, new Observer<Activities>() {
            ...
            public void onChanged(final Activities activities) {
                ...
                binding.etActivitiesTitle.setText(activities.getActivity_name());
...
//                });
                ...
                if(NetWorkUtil.isConnected(getBaseContext())){
                    orgListLive = mViewModel.getAllOrganizationListLiveNet();
                } else {
                    orgListLive = mViewModel.getAllOrganizationListLive();
                }
                orgListLive.observe(INSTANCE, new Observer<List<Organization>>() {
                    ...
                    public void onChanged(final List<Organization> organizations) {
                        ...
                        for (int i = 0; i < organizations.size(); i++) {
                            if(organizations.get(i).getId() == activities.getOrganization_id()){
                                organizationSelectPosition = i;
                            }
                            organizationList.add(organizations.get(i).getOrganization_name());
                        }
                        ...
                        binding.spOrganization.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                            ...
                            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                                ...
                                for (int i = 0; i < organizationList.size(); i++) {
                                    if(organizationList.get(position).equals(organizations.get(i).getOrganization_name())){
                                        act.setOrganization_id(organizations.get(i).getId());
                                        break;
                                    }
                                }
                            }
                ...
                binding.etActivitiesStartTime.setText(activities.getStart_time());
...
//                } else if(mViewModel.getSpUserPower()<4 && mViewModel.searchPersonInChargeOrgId(mViewModel.getUserAccount()) == act.getOrganizationId()){
                } else if(mViewModel.getSpUserPower()<4 && orgOfAct.getPerson_id() == mViewModel.getSpUserAccount()){
...
//                                String activitiesHoldOrg = binding.etActivitiesHoldOrganization.getText().toString().trim();
                                ...
                                act.setActivity_name(activitiesTitle);
                                act.setStart_time(activitiesStartTime);
                                act.setDescription(description);

                                if(NetWorkUtil.isConnected(getBaseContext())){
                                    mViewModel.updateActivitiesNet(act);
                                } else {
