...
public class AtyStudentUnionOrganizationControlView extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(NetWorkUtil.isConnected(getBaseContext())){
            organizationLiveData = mViewModel.getOrganizationLiveByIdNet(organizationId);
        } else {
        ...
        organizationLiveData.observe(this, new Observer<Organization>() {
            ...
            public void onChanged(Organization organization) {
                org = organization;
                binding.etControlOrganization.setText(organization.getOrganization_name());
                binding.etControlChargePosition.setText(organization.getPerson_in_charge());
                binding.etControlPersonInCharge.setText(String.valueOf(organization.getPerson_id()));
                ...
                if(mViewModel.getSpUserPower()<3 && mViewModel.getSpUserPower()!= org.getPerson_id()){
                ...
                } else if(mViewModel.getSpUserPower()<3 && mViewModel.getSpUserPower() == org.getPerson_id()){
                    binding.etControlOrganization.setEnabled(false);
                    binding.etControlChargePosition.setEnabled(false);
                    binding.etControlPersonInCharge.setEnabled(false);
                } else{
                ...
                build = new AlertDialog.Builder(this).setTitle(R.string.info_save).setMessage(R.string.info_check_save)
                        .setPositiveButton(R.string.info_save, new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                org.setOrganization_name(organizationName);
                                org.setPerson_in_charge(chargePosition);
                                org.setPerson_id(personInCharge);
                                org.setIsEnroll(isEnroll);
                                org.setDescription(description);

                                if(NetWorkUtil.isConnected(getBaseContext())){
                                    mViewModel.updateOrganizationNet(org);
                                } else {
