...
public interface UserMapper {
    ...
    @Insert("insert into tb_user(account,password,gender,name,phoneNumber,department,major,classes,power) " +
            "values(#{account},#{password},#{gender},#{name},#{phoneNumber},#{department},#{major},#{classes},#{power})")
    ...
    @Update("update tb_user set password=#{user.password},gender=#{user.gender}," +
            "name=#{user.name},phoneNumber=#{user.phoneNumber},department=#{user.department}" +
            ",major=#{user.major},classes=#{user.classes},power=#{user.power} where" +
            " account=#{user.account}")
    public int update(@Param("student") User user);
